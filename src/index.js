...
const minPriceReducer = () => {
  return goods.reduce((prev, item) => {
    return Math.min(prev, item.price);
  }, 6499);
};

const maxPriceReducer = () => {
  return goods.reduce((prev, item) => {
    return Math.max(prev, item.price);
  }, 0);
};
...
const toMaxSorter = () => {
  return goods.sort((a, b) => {
    ...
    // Такой метод сортирует так же как ветвление иф елса,
    // знаю как он осуществляется, решил так сократить код
    // поправь меня, как лучше использовать
  });
};
const toMinSorter = () => {
  return goods.sort((a, b) => {
